...
MODULE_DESCRIPTION("LED char device + GPIO27 control on RPi Zero W");
MODULE_VERSION("1.0");
...
static ssize_t m_write(struct file *filp, const char __user *user_buf, size_t size, loff_t *offset) {
    size_t to_copy = (size > BUF_SIZE - 1) ? BUF_SIZE - 1 : size;
...
static int __init led_init(void) {
    ...
    if (IS_ERR(led.dev_class)) goto unregister_dev;

    if (IS_ERR(device_create(led.dev_class, NULL, led.dev_num, NULL, DEVICE_NAME)))
        goto destroy_class;
